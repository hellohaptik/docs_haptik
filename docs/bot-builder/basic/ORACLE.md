...
What should the Bot Builder do after seeing entries in Section 3?
   ...
   2. The PAIR OF SIMILAR Nodes shown in this section show the possibility of wrong response being sent to the User. Hence, adequate testing needs to be done. Variants for testing can be generated by borrowing some keywords from Node B and adding them to variants present in Node A, as shown in steps below.
       ...
       d. Send such variants as messages on the Bot to test them out. If such variants go to a wrong node or show disambiguation, add these newly generated test variants to the right node. In the below image, it should be evaluated which node can the user message be added to. The process is similar to the one shown in Debugging Guidelines.
